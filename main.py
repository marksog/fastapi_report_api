...
# Paths Parmeters for workers operations # done by Admin or Pastor of a location.
class WorkersOperation(str, Enum):
    workers = "workers"
    worker = "worker"
...
# Request Data Models
# a potential/contact will grow to become a disciple and then a worker. so user_id will be same for all three
class PotentialContact(BaseModel):
    first_name: str
    last_name: str | None = None
    user_id: str                   # Unique identifier for the user generated by the system
    contact: dict
    location: str 
    notes: str | None = None
    date: str
    ...
    leader: str    # this is the logged in user who is creating the potential/contact
...
@app.get("/workers/{workers}")                      # only Admin or Pastor of a location can access this
async def get_workers(workers: WorkersOperation):
    """
    returns a list of workers
    """
    # list of all workers
    return [
        {"message": "List of workers retrieved successfully"}
    ]

@app.get("/worker/{worker_id}")
async def get_worker(worker_id: str):
    """
    returns a specific worker details
    worker_id: str is also the user_id of the worker
    """
    # a specific worker
    return {
...
@app.post("/worker/{new_worker}")
async def create_worker(new_worker: WorkersOperation, worker_contact: PotentialContact):
    """
    creates a new worker
    """
    # create a new worker
    # user_id will jusst migrate to worker_contact
    # logic to create a new worker with the provided contact details
    return {"message": "New worker created successfully", "worker_contact": worker_contact}

@app.put("/worker/{modify_worker}/{worker_id}")
async def modify_worker(modify_worker: WorkersOperation, worker_id: str, worker_contact: PotentialContact):
    """
    modifies a specific worker details
    """
    # modify a specific worker
    # logic to modify the worker's contact details
    return {"message": "Worker modified successfully", "worker_contact": worker_contact}
